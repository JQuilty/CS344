...
int main(int argc, char *argv[])
{
  ...
  //Close out file
  fclose(fp);
  //printf("Plain text is %s and it's length is %d\n", plainText, strlen(plainText));
  ...
  if (socketFD < 0) error("CLIENT: ERROR opening socket");
	...
	char* testMessage = "ConnectivityPingDec";
	...
	//	printf("Client: Buffer at 143 is %s\n", buffer);
...
while (charsWritten <= characterCount)
{
	...
	//printf("Buffer bfore while loop is %s\n", buffer);
